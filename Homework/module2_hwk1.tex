...
Let $\mathcal F$ be a convex class of absolutely continuous distribution functions which contains all uniform densities. If $X = (X_1, ..., X_n)$ are iid $F \in \mathcal{F}$, then $T(X) = (X_{(1)},...X_{(n)})$ is 
\begin{description}
\item[(a)] sufficient for $F in \mathcal F$
%https://stats.stackexchange.com/questions/144646/proof-that-n-order-statistics-are-sufficient-for-a-sample-of-size-n
\item[(b)] complete for $F in \mathcal F$
%https://sites.stat.washington.edu/jaw/COURSES/580s/581/LECTNOTES/ch13.ps - Theorem 5.3
\end{description}
\end{homeworkProblem}
